		...
		<td>
			...
			<div id="content-table-inner">
				...
				<?php $form = ActiveForm::begin([
					'action'=>Url::to(['kandidat/update-test', 'id' =>$test->id,'kandidatid'=>$kandidat->id ]),
					...
					<?= $form->field($kandidat, 'kandidatid')->textInput(['readonly' => true])->label('ID Kandidat') ?>
					<?= $form->field($kandidat, 'namalengkap')->textInput(['readonly' => true])->label('Nama Kandidat') ?>
					<?= $form->field($test, 'name_test')->textInput(['autofocus' => true])->label('Nama Test') ?>
					<?= $form->field($test, 'result_test') ->label('Nilai Test')?>
					<?= $form->field($test, 'keterangan') ->label('Keterangan')?>
