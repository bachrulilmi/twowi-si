...
use yii\helpers\Url ;
$this->title = 'Edit Kandidat Kandidat';
?>
		...
		<td>
			...
			<div id="content-table-inner">
				...
				<?php $form = ActiveForm::begin([
					'action'=>Url::to(['kandidat/update-all', 'id' =>$model->id ]),
							...
							<?= $form->field($model, 'autoca')->dropdownList([
								...
								<?= $form->field($model, 'mengemudi')->dropdownList([
									...
									<?= $form->field($model, 'jabatan')->checkboxList([
										...
										<div class="form-group field-kandidat-fotokandidat required">
											<label class="col-lg-1 control-label" for="kandidat-fotokandidat">Foto Kandidat</label>
											<div class="col-lg-3">
												<input type="hidden" name="Kandidat[fotokandidat]" value="<?= $model->fotokandidat ?>">
												<input type="file" id="kandidat-fotokandidat" name="Kandidat[fotokandidat]" value="<?= $model->fotokandidat ?>">
												<button type="button" onclick="window.open('<?= Url::base() . "/foto/".$model->fotokandidat ?>')" class="btn btn-success">Download Lampiran Sebelumnya</button>
											</div>
